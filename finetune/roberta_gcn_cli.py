...
def main():
    ...
    if not args.tag_mspan:
        train_itr = DropBatchGen(args, data_type='drop', data_mode="train", tokenizer=tokenizer)
        dev_itr = DropBatchGen(args, data_type='drop', data_mode="dev", tokenizer=tokenizer)
    else:
        train_itr = TDropBatchGen(args, data_type='drop', data_mode="train", tokenizer=tokenizer)
        dev_itr = TDropBatchGen(args, data_type='drop', data_mode="dev", tokenizer=tokenizer)
    num_train_steps = int(
    ...
    if args.cuda:
        network.cuda()
    print("Load from pre path {}.".format('../pretrain/pretrain_best.pt'))
    network.load_state_dict(torch.load('../pretrain/pretrain_best.pt'))
